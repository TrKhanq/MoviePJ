// Ticket data source for booking page
...
function initTicketsStorage({ forceReset = false } = {}) {
  try {
    if (forceReset) {
      localStorage.setItem(TICKETS_STORAGE_KEY, JSON.stringify(ticketsData));
      return [...ticketsData];
    }
...
export function getTickets() {
  return initTicketsStorage();
}

export function setTickets(tickets) {
  const next = Array.isArray(tickets) ? tickets : [];
  try {
...
// Seed localStorage as soon as module is imported
initTicketsStorage();
